...
function ProductMaterialEdit(props) {
  ...
  const loadProducts = async () => {
    const resultproducts = await axios.get("https://localhost:44330/api/Products/GetAllProducts");
    setProducts(resultproducts.data);
  }

  const loadMaterials = async () => {
    const resultmaterials = await axios.get("https://localhost:44330/api/Materials/GetAllMaterials");
  ...
  async function update(e) {
    e.preventDefault();
    await axios.put(`https://localhost:44330/api/ProductMaterials/UpdateProductMaterial/${id}`,
      bodyParameters
      // ,{
      //     headers: {
